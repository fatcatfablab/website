...
describe("theme and homepage hero regression contract", () => {
  ...
  it("stages the original eager group photo before revealing the local video", () => {
    ...
    expect(script).toContain("poster.decode()");
    expect(script).toContain('matchMedia("(prefers-reduced-motion: reduce)")');
    expect(script).toContain("await video.play()");

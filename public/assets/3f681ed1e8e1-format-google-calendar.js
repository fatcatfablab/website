...
var dateStart = getDateInfo(result.start.dateTime || result.start.date),
            ...
            if(descriptionLines.length > 4){
                descriptionFormatted = description.split('\n')[3];
            }
            descriptionFormatted = descriptionFormatted.replace(/(\s)(http:\/\/[^\s]+)(\s)/g, '$1<a href="$2">$2</a>$3');
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
                ...
                output = output.concat('<span class="description">' + descriptionFormatted + '</span>');
            } else {
                if ((format[i + 1] === '*summary*' && summary !== '') ||
